...
The [built-in themes] that are included with Docums provide support for
...
themes. For a third-party theme to make use of Docums' translation tools and
...
Docums' `setup.py` script as described below to assist with the process of
...
built-in themes (`docums` and `readthedocs`), you can easily contribute a
...
2. [Add a translation](#translating-the-docums-themes) for every text
...
for the Spanish `es` language to the `docums` theme, run the following command:

```bash
python setup.py init_catalog -t docums -l es
```

The above command will create the following file structure:

```text
docums/themes/docums/locales
├── es
│   └── LC_MESSAGES
│       └── messages.po
```
...
translation](#translating-the-docums-themes) for every text placeholder in the
localized catalog.
...
2. [Translate](#translating-the-docums-themes) the newly added translatable
...
To update the `fr` translation catalog of the `docums` theme, use the following
command:

```bash
python setup.py update_catalog -t docums -l fr
```

You can now move on to the next step and [add a translation] for every updated
text placeholder in the localized catalog.

[add a translation]: #translating-the-docums-themes

### Translating the Docums themes
...
the `es` translation for the `docums` theme.

```bash
python setup.py compile_catalog -t docums -l es
```
...
Then modify the `docums.yml` file at the root of the project to test the new
...
Finally, run `docums serve` to check out your new localized version of the theme.
...
actual text translation `messages.po` files (the rest is ignored by git).
    ...
    the `locale` setting in the `docums.yml` file before submitting your
    changes.
