...
the project directory named `docums.yml`. You can specify another path for it
by using the `-f`/`--config-file` option (see `docums build --help`).
...
Docums site will be within a subdirectory of a domain, be sure to include that
subdirectory in the setting (`https://example.com/foo/`).

This setting is also used for `docums serve`: the server will be mounted onto a
...
etc), the branch, and the docs directory itself. Docums concatenates `repo_url`
...
All paths must be relative to the `docums.yml` configuration file. See the
...
how Docums determines the page title of a document.
...
However, sometimes the Docums site is hosted in a subdirectory of a project's
...
note that the `site_url` indicates that the Docums site is hosted in the `/foo/`
...
`User Guide` points to a local Docums page.
...
Set a list of templates in your `docs_dir` to be built by Docums. To see more
about writing templates for Docums read the documentation about [custom themes]
...
Determines the address used when running `docums serve`. Must be of the format
`IP:PORT`.

Allows a custom default to be set without the need to pass it through the
`--dev-addr` option every time the `docums serve` command is called.
...
Docums uses the [Python Markdown][pymkd] library to translate Markdown files
...
extensions beyond the ones that Docums uses by default (`meta`, `toc`, `tables`,
and `fenced_code`).
...
If the `plugins` config setting is defined in the `docums.yml` config file, then
...
**default**: `['search']` (the "search" plugin included with Docums).

#### Search

A search plugin is provided by default with Docums which uses [lunr.js] as a
...
!!! Warning
    ...
    resources. Generally, it is best to keep each instance of Docums to a single
    language.
...
When running `docums build`, the file at `foo/docums.yml` would be passed in as
the configuration file. Docums will then parse that file, retrieve and parse the
parent file `base.yml` and deep merge the two. This would result in Docums
...
!!! warning
    ...
    primary configuration file and Docums does not alter the paths when merging.
