...
!!! note
    ...
    `.foo.md` or `.bar/baz.md`) are ignored by Docums, which matches the
...
Docums to the built site unaltered. See [how to link to images and media]
...
Docums includes some extensions by default, which are highlighted below.
...
!!! warning
    ...
    are adjusted by Docums to ensure they are always relative to the page. Absolute
...
: The "title" to use for the document.
    ...
    Upon finding a title for a page, Docums does not continue checking any
