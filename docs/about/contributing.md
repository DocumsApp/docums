# Contributing to Docums

An introduction to contributing to the Docums project.

The Docums project welcomes, and depends, on contributions from developers and
...
[README](https://github.com/khanhduy1407/docums/blob/master/README.md) file in our
GitHub repository.
...
Please include as much detail as you can. Let us know your platform and Docums
...
Docums you can do so with the following command. This can be useful if you
want to provide feedback for a new feature or want to confirm if a bug you
have encountered is fixed in the git master. It is **strongly** recommended
that you do this within a [virtualenv].

```bash
pip install https://github.com/khanhduy1407/docums/archive/master.tar.gz
```
...
This will install Docums in development mode which binds the `docums` command
...
Then the test suite can be run for Docums by running the command `tox` in the
root of your Docums repository.
...
If you're considering a large code contribution to Docums, please prefer to
...
When installed with `i18n` support (`pip install docums[i18n]`), Docums allows
...
`pot` file of the `docums` theme, run the following command:

```bash
python setup.py extract_messages -t docums
```
