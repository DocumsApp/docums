...
A Guide to installing, using and creating Docums Plugins
...
using a plugin which comes with Docums, then it was installed when you installed
Docums. However, to install third party plugins, you need to determine the
...
Like Docums, plugins must be written in Python. It is generally expected that
each plugin would be distributed as a separate Python module, although it is
possible to define multiple plugins in the same module. At a minimum, a Docums
Plugin must consist of a [BasePlugin] subclass and an [entry point] which
points to it.

### BasePlugin

A subclass of `docums.plugins.BasePlugin` should define the behavior of the plugin.
...
:   A tuple of configuration validation instances. Each item must consist of a
    ...
    `docums.config.config_options.BaseConfigOption` or any of its subclasses.
    ...
    provided in `docums.config.config_options` or a third party subclass defined
    in the plugin.

    Any settings provided by the user which fail validation or are not defined
    in the `config_scheme` will raise a `docums.config.base.ValidationError`.
...
:   Loads configuration from a dictionary of options. Returns a tuple of
    `(errors, warnings)`. This method is called by Docums during configuration
...
:   Optional methods which define the behavior for specific [events]. The plugin
    ...
    additional keywords are provided to an event in a future version of Docums,
...
:   The `build_error` event is called after an exception of any kind
    is caught by Docums during the build process.
    Use this event to clean things up before Docums terminates. Note that any other
...
Docums defines four error types:

#### `docums.exceptions.DocumsException`

:   The base class which all Docums exceptions inherit from. This should
    not be raised directly. One of the subclasses should be raised instead.

#### `docums.exceptions.ConfigurationError`
...
#### `docums.exceptions.BuildError`

:   This error may be raised by Docums during the build process. Plugins should
not raise this error.

#### `docums.exceptions.PluginError`

:   A subclass of `docums.exceptions.BuildError` which can be raised by plugin
    events.
...
message. Therefore, Docums will catch any of the errors listed above, retrieve
...
from Docums) and each must register as a Plugin via a setuptools `entry_point`.
...
tell Docums to use it via the config.
